...
const MentorUploadProject = () => {
    const [formData,setFormData]=useState({title:"", description:"", intershipType:"", number_of_opening:""})
    ...
    const GetCategoryData = async ()=>{
        try {
          ...
          let datas = resp.data.data.map((data)=>{
            return data.subcategory
          })
          const result = datas.reduce((r, e) => (r.push(...e), r), [])
    ...
    const FormSubmitHandler=async()=>{
        try {
            const resp = await authFetch.post('/api/mentor/intership',{title:formData.title,description:formData.description,
            tags:subCategoryData,intershipType:formData.intershipType,number_of_opening:formData.number_of_opening});
...
export default MentorUploadProject;
...
const Mentor_Request = () => {
  ...
  const GetMentorRequest = async () => {
    try {
      const resp = await authFetch.get(`/api/mentor/get-project`);
  ...
  const AcceptProjectRequest = async (projectId, studentid) => {
    try {
      const resp = await authFetch.patch(`/api/mentor/get-project/${projectId}`, { studentId: studentid });
      setMentorRequest(resp.data.data)
      GetMentorRequest()
    } catch (error) {
      console.log(error)
  ...
  const DeclineProjectRequest = async (projectId, studentid) => {
    try {
      const resp = await authFetch.patch(`/api/mentor/get-project/decline/${projectId}`, { studentId: studentid });
      setMentorRequest(resp.data.data)
      GetMentorRequest()
    } catch (error) {
      console.log(error)
              ...
              return <> <div key={data._id} className="container mx-auto sm:px-8">
                <div className="flex justify-between border border-5 p-6 rounded-md mb-5">
                  <div className='flex justify-start ml-5'>
                    ...
                    <div className='ml-5'>
                      <h4 onClick={() => navigate(`/auth/mentor/view-project/${request._id}`)} className="text-blue-500 font-semibold text-2xl cursor-pointer underline">{request.title.toUpperCase()}</h4>
                      <p onClick={() => navigate(`/auth/campus/view-profile/${request.userId._id}`)} className="text-blue-500 text-lg cursor-pointer"><span className='text-gray-900 font-semibold'>Company Name :</span> {request.userId.name.toUpperCase()}</p>

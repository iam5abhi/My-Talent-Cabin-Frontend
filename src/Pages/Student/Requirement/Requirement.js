...
const Requirement = () => {
    ...
    const GetRequirementData = async ()=>{
        try {
          const resp = await authFetch('/api/student/get-job-application');
            ...
            <div className="grid grid-cols-3 gap-4 px-6  py-6">
                {!requirementData?null:requirementData.map((datas)=>{
                    ...
                    <span className="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100">
                        ...
                        <button type="button" onClick={()=>ApplySubmitData(datas._id)} className="text-white text-end bg-orange-600 hover:bg-orange-400 focus:ring-4 focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none">Apply</button>
                    </span>
                    </div>
                })}
            </div> 

...
const PersonalInformation = ({Data}) => {
    ...
    const [updateProfileData,setUpdateProfileData]=useState({dob:"",family_occupation:"",gender:""})
    ...
    const GetProfileData =async()=>{
        try {
          ...
          setUpdateProfileData({...updateProfileData,dob:resp.data.data.dob,family_occupation:resp.data.data.family_occupation,gender:resp.data.data.gender})
        } catch (error) {
          console.log(error)
        }
      } 
      ...
      const SubmitProfileData =async(event)=>{
        ...
        try {
          const resp = await authFetch.patch('/api/student/update-personal-information',updateProfileData);
          setMessage({message:resp.data.message,type:true})
          GetProfileData() 
          setTimeout(() => {
            Data()
            navigate('/auth/student/profile/education')
  ...
  return (
        ...
        <section>
            ...
            <form onSubmit={SubmitProfileData}>
                ...
                <div className="mb-6  ">
                <label htmlFor="bordered-checkbox-1" className="w-full py-4  text-base font-medium text-gray-900 dark:text-gray-300">Family
                    Occupation &nbsp;</label>
                <input id="default-checkbox" type="checkbox" name='family_occupation' onChange={ProfileHandler} checked={updateProfileData.family_occupation==="salaried"} value="salaried" className="w-4 h-4 text-orange-600 bg-gray-100 border-gray-300 rounded focus:ring-orange-500 dark:focus:ring-orange-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                <label htmlFor="bordered-checkbox-1" className="w-full py-4 ml-2 text-base  text-gray-900 dark:text-gray-300">Salaried
                    &nbsp;</label>
                <input id="default-checkbox" type="checkbox" name='family_occupation' onChange={ProfileHandler} value="agriculture" checked={updateProfileData.family_occupation==="agriculture"} className="w-4 h-4 text-orange-600 bg-gray-100 border-gray-300 rounded focus:ring-orange-500 dark:focus:ring-orange-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                <label htmlFor="bordered-checkbox-1" className="w-full py-4 ml-2 text-base   text-gray-900 dark:text-gray-300">Agriculture
                    &nbsp;</label>
                <input id="default-checkbox" type="checkbox" name='family_occupation' onChange={ProfileHandler} value="others" checked={updateProfileData.family_occupation==="others"} className="w-4 h-4 text-orange-600 bg-gray-100 border-gray-300 rounded focus:ring-orange-500 dark:focus:ring-orange-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                <label htmlFor="bordered-checkbox-1" className="w-full py-4 ml-2 text-base   text-gray-900 dark:text-gray-300">Others
                    &nbsp;</label>
                </div>
                <div className="mb-6">
                <label htmlFor="dob" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">DOB</label>
                <input type="date" id="text" name='dob' max={`${fullYear}-12-31`} onChange={ProfileHandler} value={updateProfileData.dob} className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-orange-600 focus:border-orange-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500" placeholder required />
                </div>
                <div className="mb-6  ">
